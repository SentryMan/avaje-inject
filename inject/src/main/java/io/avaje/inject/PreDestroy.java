 ...
 *
 * <p>Note that we can equally use any <code>PreDestroy</code> annotation - so we can use the one
 * from <code>javax.annotation</code>, <code>jakarta.annotation</code> or this one.
 *
 * <p>The method annotated with <code>PreDestroy</code> is typically used to release resources that
 * it has been holding.
 *
 * <p>The method on which the <code>PreDestroy</code> annotation is applied must fulfill the
 * following criteria:
 *
 * <ul>
 *   <li>The method must not have any parameters.
 *   <li>The method may be public, protected or package private.
