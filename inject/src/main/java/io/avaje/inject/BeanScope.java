 ...
 *
 * <p>The beans have singleton scope, support lifecycle methods for postConstruct and preDestroy and
 * are created (wired) via dependency injection.
 *
 * <h3>Create a BeanScope</h3>
 *
 * <p>We can programmatically create a BeanScope via {@code BeanScope.builder()}.
 *
 * <pre>{@code
 * // create a BeanScope ...
 *
 ...
 *
 * <p>We can supporting external dependencies when creating the BeanScope. We need to do 2 things.
 * we need to specify these via
 *
 * <ul>
 *   <li>1. Specify the external dependency via {@code @InjectModule(requires=...)}. Otherwise at
 *       compile time the annotation processor detects it as a missing dependency and we can't
 *       compile.
 *   <li>2. Provide the dependency when creating the BeanScope
 * </ul>
 *
 * <p>For example, given we have Pump as an externally provided dependency.
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>When we build the BeanScope provide the dependency via {@link BeanScopeBuilder#bean(Class,
 * Object)}.
 *
 * <pre>{@code
...
public interface BeanScope extends AutoCloseable {
   ...
   *
   * <ul>
   *   <li>Provide external dependencies
   *   <li>Specify a parent BeanScope
   *   <li>Specify specific modules to wire
   *   <li>Specify to include a shutdown hook (to fire preDestroy lifecycle methods)
   *   <li>Use {@code forTesting()} to specify mocks and spies to use when wiring tests
   * </ul>
   *
   * <pre>{@code
   * // create a BeanScope ...
  ...
  /**
   * Return the list of beans that have an annotation. The annotation must have a @Retention policy
   * of RUNTIME
   *
   * <pre>{@code
   ...
   *
   * <p>The priority annotation will typically be either <code>javax.annotation.Priority</code> or
   * <code>jakarta.annotation.Priority</code>.
   *
   ...
   *
   * <p>Beans with no qualifier name get a generated unique key to use instead.
   */
   ...
   *
   * <p>The bean entries include entries from the parent scope if it has one.
   *
