 ...
 *
 * <p>Use {@code requires} to specify dependencies that will be provided externally.
 *
 * <pre>{@code
 ...
 *
 * <p>When using custom scopes we can have the case where we desire one scope to depend on another.
 * In this case we put the custom scope annotation in requires.
 *
 * <p>For example lets say we have a custom scope called {@code StoreComponent} and that depends on
 * {@code QueueComponent} custom scope.
 *
 * <pre>{@code
 * @Scope
...
public @interface InjectModule {
  ...
  /**
   * Optimizes module wiring by enforcing multi-module wiring checks at compile-time. Will throw a
   * compilation error if all inter-module {@link InjectModule#requires requires} dependencies are
   * not satisfied.
   *
   ...
   *
   * <p>Set this to true when some other library is using <code>@Singleton</code> and we want
   * avaje-inject to be completely independent of that by ignoring the standard <code>@Singleton
   * </code>.
   *
   * <p>We instead use <code>@Component</code> instead of <code>@Singleton</code>.
   */
   ...
   *
   * <p>Instead of listing each and every dependency in {@code requires} we can use this to specify
   * that any required dependency that is in these packages is expected to be provided by another
   * module.
   *
   * <p>Use this rather than {@code requires} when there are lots of required dependencies, and we
   * don't want to list each one in {@code requires} and {@code provides}.
   */
   ...
   *
   * <p>When a module is generated for a custom scope this is set to link the module back to the
