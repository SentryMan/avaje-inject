...
class MyAutoNameTest {
  ...
  void test() {
    ...
    try (final BeanScope beanScope = BeanScope.builder().build()) {
      ...
      final MyGeneric<?> generic2 =
          beanScope.get(MyAutoNameFactory$DI.TYPE_MyGenericString, "genericString");
      final MyGeneric<?> generic3 =
          beanScope.get(MyAutoNameFactory$DI.TYPE_MyGenericSome, "genericOther");
