...
public abstract class TestBeanScope {
   ...
   *
   * <p>That is, beans created using {@code @TestScope} are all in the global "test scope" which is
   * a parent to this scope.
   *
   * <pre>{@code
   ...
   *
   * <p>If the BeanScope has already been created then that shared scope instance is returned. This
   * will be the same BeanScope as is used by {@code @InjectTest}.
   *
   * <p>The global test scope is closed when either JUnit closes or on JVM shutdown.
   *
   * <pre>{@code
  ...
  /**
   * Build and return a new BeanScope for {@code @TestScope} beans. This builds the beans in the
   * test scope every time it is called.
   *
   * <p>Generally, we DO NOT want to use this method but use {@link #initialise()} instead.
   *
