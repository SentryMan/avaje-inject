 ...
 *
 * <p>Supports injection for fields annotated with <code>@Mock, @Spy, @Captor, @Inject</code>.
 */
public final class InjectExtension
  ...
  private GlobalTestBeans.Beans getClassScope(
      ExtensionContext context, GlobalTestBeans.Beans globalTestBeans) {
    Class<?> testClass = context.getRequiredTestClass();
  ...
  public void beforeEach(final ExtensionContext context) {
    ...
    if (metaInfo.hasInstanceInjection()) {
      ...
      final GlobalTestBeans.Beans parentBeans =
          metaInfo.hasStaticInjection() ? getClassScope(context, globalTestBeans) : globalTestBeans;
      AutoCloseable metaScope =
          metaInfo.buildForInstance(parentBeans, context.getRequiredTestInstance());
  ...
  public void afterEach(ExtensionContext context) {
    Method testMethod = context.getRequiredTestMethod();
    final AutoCloseable beanScope =
        (AutoCloseable) context.getStore(INJECT_NS).remove(BEAN_SCOPE + testMethod);
    if (beanScope != null) {
      try {
        beanScope.close();
      } catch (Exception e) {
  ...
  public void afterAll(ExtensionContext context) {
    ...
    final AutoCloseable scope =
        (AutoCloseable) context.getStore(INJECT_NS).remove(BEAN_SCOPE + testClass);
    if (scope != null) {
      try {
        scope.close();
      } catch (Exception e) {
